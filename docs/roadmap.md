...
- [x] **Morning Ritual** — `/routine`: a calm, timed 6am→7am flow — voice-first check-in (reuses the morning model) → 5-min guided stretch sequence → 30–45 min walk with a soft target band, then a completion step that logs durations + walking steps and reads a spoken daily briefing. A live master timer runs the hour. Completions feed a ritual streak (`calcRoutineStreak`, shown on the dashboard). The 6am morning push points here.
- [x] **AI voice coach (back-and-forth)** — `VoiceChat` component: hands-free spoken conversation during the ritual. Listens via Web Speech, replies via `/api/coach` (with coach memory), speaks back. Voice is **premium-first, offline-safe**: `/api/tts` uses OpenAI's natural voice (`OPENAI_TTS_VOICE`, default `sage`) when a key is set and returns 204 otherwise, so `useSpeech` falls back to the browser's `speechSynthesis`. Mid-walk check-ins and the closing briefing use the same engine.
- [x] **Walking steps** — tracked daily metric (manual from Oura/Apple), wired into vitals, correlations (Steps vs Pressure/Elevator), the Weekly Review, and CSV export.
